...
async function loadStyles() {
    ...
    const migrationResult = await presetStorage.migrateLegacyData();
    if (migrationResult.migrated && migrationResult.count > 0) {
        console.log(`Migrated ${migrationResult.count} legacy presets`);
    }
    
    // ===== STEP 2: Load factory presets (from cache or file) =====
    const hasFactoryCache = await presetStorage.hasFactoryPresetsCache();
    
    if (!hasFactoryCache) {
        // First run: fetch from presets.json and cache
        try {
            const response = await fetch('./presets.json');
            if (response.ok) {
                const jsonPresets = await response.json();
                // Assign IDs to factory presets
                const factoryWithIds = jsonPresets.map((p, idx) => ({
                    ...p,
                    id: `factory_${p.name.toLowerCase().replace(/\s+/g, '_')}`
                }));
                await presetStorage.setFactoryPresets(factoryWithIds);
                factoryPresets = factoryWithIds;
                console.log('Factory presets loaded and cached');
            }
        } catch (e) {
            console.error('Error loading factory presets:', e);
            factoryPresets = [];
        }
    } else {
        // Use cached factory presets
        factoryPresets = await presetStorage.getFactoryPresets();
        console.log('Factory presets loaded from cache');
    }
    
    // ===== STEP 3: Check for factory preset updates =====
    try {
        const response = await fetch('./presets.json');
        if (response.ok) {
            const jsonPresets = await response.json();
            const currentVersion = await presetStorage.getFactoryCacheVersion();
            const newVersion = presetStorage._getFileVersion(jsonPresets);
            
            if (currentVersion !== newVersion) {
                // Factory presets updated - update cache
                const factoryWithIds = jsonPresets.map((p, idx) => ({
                    ...p,
                    id: `factory_${p.name.toLowerCase().replace(/\s+/g, '_')}`
                }));
                await presetStorage.setFactoryPresets(factoryWithIds);
                factoryPresets = factoryWithIds;
                console.log('Factory presets cache updated');
        ...
        console.log('Could not check for preset updates:', e);
    }
    
    // ===== STEP 4: Load user presets from IndexedDB =====
    const userPresets = await presetStorage.getUserPresets();
    console.log(`Loaded ${userPresets.length} user presets`);
    
    // ===== STEP 5: Merge factory + user presets =====
    // Factory presets first, then user presets alphabetically
    const userPresetNames = new Set(userPresets.map(p => p.name));
    
    // Filter out factory presets that have been deleted by user
    const activeFactoryPresets = factoryPresets.filter(p => !userPresets.find(up => up.name === p.name && up.deleted));
    
    // Merge: factory + user presets
    CAMERA_PRESETS = [...activeFactoryPresets, ...userPresets];
    ...
    // ===== STEP 7: Load last used preset =====
    loadLastUsedStyle(); 
    
    // ===== STEP 8: Load resolution settings =====
    loadResolution();
    
    // ===== STEP 9: Load visible presets =====
    ...
    // Clean up visible presets - remove any preset names that no longer exist
...
async function deleteCustomPreset() {
  ...
  // Remove from IndexedDB (using new storage)
  await presetStorage.deleteUserPreset(preset.id);
...
if (startBtn) {
  ...
  // Save Copy button - creates a new copy of the current preset
  const presetBuilderSaveCopy = document.getElementById('preset-builder-save-copy');
  if (presetBuilderSaveCopy) {
    presetBuilderSaveCopy.addEventListener('click', async () => {
      const name = document.getElementById('preset-builder-name').value.trim();
      const categoryInput = document.getElementById('preset-builder-category').value.trim();
      const prompt = document.getElementById('preset-builder-prompt').value.trim();
      ...
      let counter = 2;
      while (CAMERA_PRESETS.some(p => p.name.toUpperCase() === copyName.toUpperCase())) {
      ...
      // Create new preset object (always creates new, never overwrites)
      const newPreset = {
      ...
      // Show success message
      alert(`Preset "${copyName}" saved as copy!`);
      
      // Clear form and go back (but don't change editing state - keep original intact)
      clearPresetBuilderForm();
      hidePresetBuilderSubmenu();
    });
  }
